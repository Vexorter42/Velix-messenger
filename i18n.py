...
def t(text, /, **values):
    ...
    Ключ принимается только позиционно: среди подстановок встречается
    и {text}, а она бы столкнулась с именем самого параметра.
    """
...
SERVER_MESSAGES = {
    ...
    "group_needs_title": "У группы должно быть название.",
    "group_only": "Позвать можно только в группу.",
    ...
    "group_needs_members": "Выберите, кого позвать в группу.",
...
ENGLISH = {
    ...
    "Копировать фото": "Copy photo",
    "Копировать файл": "Copy file",
    "Закрепить": "Pin",
    "Открепить": "Unpin",
    "Переслать": "Forward",
    "Куда переслать": "Forward to",
    "Переслано от {name}": "Forwarded from {name}",
    "Закреплённое сообщение": "Pinned message",
    "Скопировано": "Copied",
    "Записываю голос…": "Recording voice…",
    "Записываю кружочек…": "Recording a circle…",
    "осталось": "left",
    "Записать не вышло: {error}": "Could not record: {error}",
    "устройство занято": "the device is busy",
    ...
    "Звук о новом сообщении": "Sound on a new message",
    ...
    "Позвать можно только в группу.": "You can only invite people to a group.",
    ...
    "Выберите, кого позвать в группу.": "Choose who to invite to the group.",
