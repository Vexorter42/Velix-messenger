...
| 🎤 **Voice and circles** | One button for both: a tap switches it between voice and circle, a press-and-hold starts recording. A voice message arrives as a waveform that shows where the speech is, and plays back faster — one and a half or two. A circle plays right there in the conversation, round, and shows its first frame before you tap it. |
...
| `mediatools.py`, `tidy-media.py` | A voice waveform, and circles brought to one shape |
