...
#
# Телефон пишет кружочек как 720 на 480, но с пометкой, что пиксель у него
# вытянутый. Считать такие пиксели квадратными — значит показать лицо шире,
# чем оно есть; проверяем, что пометку учитывают.

class Пометка:
    """Кадр и проигрыватель ровно настолько, насколько их спрашивает коробка."""

    def __init__(self, размер, отношение):
        self._размер = размер
        self._отношение = отношение

    def get_size(self):
        return self._размер

    def get_metadata(self):
        return {"aspect_ratio": self._отношение}


def видимый(размер, отношение):
    пустая = videoplayer.VideoBox.__new__(videoplayer.VideoBox)
    пустая.player = Пометка(размер, отношение)
    return пустая._видимый_размер(Пометка(размер, отношение))


check("aspect-square-pixels-unchanged", видимый((720, 480), (1, 1)) == (720, 480))
check("aspect-wide-pixels-widen-the-frame",
      видимый((720, 480), (32, 27)) == (853, 480), видимый((720, 480), (32, 27)))
check("aspect-nonsense-ignored", видимый((720, 480), (0, 0)) == (720, 480))
