...
@step
def look_at_long():
    """Ленте группы полагается быть выше окна — иначе проверять нечего.

    Числа тут нарочно нет: лента рисует хвост в ЛЕНТА_СРАЗУ сообщений, а не
    всё, что пришло, так что её высота зависит от настройки, а не от того,
    сколько сообщений в группе. Важно одно — что она прокручивается, то есть
    выше видимой части.
    """
    высокая = (область() or [0, 0, 0, 0])[3]
    запомнили["высокая"] = высокая
    видно = полотно().winfo_height()
    check("blank-long-tall", высокая > max(видно, 900),
          f"лента группы вышла невысокой: {высокая} при окне {видно}")
